...
# save txt file
def save_txt(t,Q1,Q2,T1,T2,K1s,K2s,K3s,tau12s,tau3s):
    data = np.vstack((t,Q1,Q2,T1,T2,K1s,K2s,K3s,tau12s,tau3s))  # vertical stack
    data = data.T                 # transpose data
    top = 'Time (sec), Heater 1, Heater 2, ' \
        + 'Temperature 1, Temperature 2,K1s,K2s,K3s,tau12s,tau3s' 
    np.savetxt('{0}MHE_data.txt'.format(fname),data,delimiter=',',header=top,comments='')
...
for i in range(n):
    if i%50==0:
        Q1s[i:i+50] = random.random() * 100.0
    if (i+20)%50==0:
        Q2s[i:i+50] = random.random() * 100.0
...
try:
    ...
    a.Q1(0)
    a.Q2(0)
    save_txt(tm,Q1s,Q2s,T1m,T2m,K1s,K2s,K3s,tau12s,tau3s)
    # Save figure
